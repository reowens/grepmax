...
| Doc | Status Snapshot |
|-----|-----------------|
| [Mcp Search Scope Leak](archived/mcp-search-scope-leak.md) | Archived: Fix A+B shipped. MCP now resolves the launch cwd to an exact registered project or registered ancestor before searching, and out-of-project default semantic search errors instead of silently searching the whole index. Fresh MCP smoke from `qsys/qsys-training` and `qsys/docs` passed: default search stayed qsys-scoped, while `scope:"all"` and `projects:"platform"` returned platform results. Source-mode workers now use an absolute `tsx` preload so dev MCP processes launched outside the gmax repo can boot query workers. |
