...
const DATASETS: Record<string, OssDataset> = {
  express: {
    name: "express",
    projectRoot: path.join(process.env.HOME ?? "", "Development/sandbox/bench-fixtures/express"),
    cases: EXPRESS_CASES,
  },
  lodash: {
    name: "lodash",
    projectRoot: path.join(process.env.HOME ?? "", "Development/sandbox/bench-fixtures/lodash"),
    cases: LODASH_CASES,
  },
  platform: {
    name: "platform",
    projectRoot: path.join(process.env.HOME ?? "", "Development/beyond/platform"),
...
async function runDataset(
  ...
  for (const c of ds.cases) {
    ...
    const res = await searcher.search(c.query, topK, { rerank }, undefined, pathPrefix);
